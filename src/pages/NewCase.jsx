...
import { ArrowLeft, ArrowRight, FileText, Upload, X, Sparkles, Check, Shield, User, Phone, Plus, Trash2, AlertCircle, Loader2, MapPin, Clock, Calendar, Download } from 'lucide-react';
...
function Field({ label, required, error, children, className = '' }) {
  return (
    <div className={className}>
      ...
      {children}
      {error && <p className="flex items-center gap-1 mt-1 text-[11px] text-red-500"><AlertCircle size={11} /> {error}</p>}
    </div>
  );
}
...
function StepPill({ n, label, active, done }) {
  return (
    <div className="flex flex-col items-center gap-1.5" style={{ minWidth: 72 }}>
      <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 ${done ? 'bg-green-500 text-white shadow-lg shadow-green-200' : active ? 'bg-gradient-to-r from-orange-400 to-green-500 text-white shadow-lg shadow-orange-200' : 'bg-gray-100 text-gray-400'}`}>{done ? <Check size={16} strokeWidth={2.5} /> : n}</div>
      <span className={`text-[11px] font-medium whitespace-nowrap transition-colors ${active ? 'text-orange-500' : done ? 'text-green-500' : 'text-gray-400'}`}>{label}</span>
    </div>
  );
}
...
Return ONLY the JSON. No other text.`;
  ...
  const analyzeWithAI = async () => {
    if (!form.description.trim()) { setErrors(p => ({ ...p, description: 'Write a description first' })); return; }
    setAiLoading(true);
    try { const sections = await window.crimeGPT.suggestSections(form.description); setAiSections({ summary: sections }); }
    catch { setAiSections({ error: true }); }
    setAiLoading(false);
  };
  ...
  const handleFiles = (e) => { const files = Array.from(e.target.files || []).filter(f => f.size <= 10 * 1024 * 1024); setForm(p => ({ ...p, evidenceFiles: [...p.evidenceFiles, ...files.map(f => ({ name: f.name, size: f.size, url: URL.createObjectURL(f), file: f }))] })); };
  const removeFile = (i) => setForm(p => ({ ...p, evidenceFiles: p.evidenceFiles.filter((_, idx) => idx !== i) }));

  const handleSave = async () => {
    setSaving(true);
    try {
      const evidenceImages = await Promise.all(form.evidenceFiles.map(async (f) => {
        const base64 = await new Promise(r => { const reader = new FileReader(); reader.onload = () => r(reader.result.split(',')[1]); reader.readAsDataURL(f.file); });
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      const result = await window.crimeGPT.registerCase({
        ...
        officer_name: user?.fullName || 'Unknown', officer_badge: user?.badgeNumber || '', officer_rank: user?.rank || 'IO',
        complainant: { full_name: form.complainantName, phone: form.complainantPhone, address: form.complainantAddress, id_proof_type: form.complainantIdType, id_proof_number: form.complainantIdNumber },
        accused: form.accusedName.trim() ? [{ full_name: form.accusedName, alias: form.accusedAlias, father_name: form.accusedFatherName, age: form.accusedAge, physical_description: form.accusedDescription }] : [],
        witnesses: form.witnessName.trim() ? [{ full_name: form.witnessName, phone: form.witnessPhone, statement: form.witnessStatement }] : [],
        seized_items: form.seizedItems.filter(i => i.item.trim()), sections, evidence_images: evidenceImages,
      });

      if (result.success) {
        const { renderFIR } = await import('../../electron/doc/FIR.js');
        const firHTML = renderFIR({
          fir_number: result.fir_number, fir_year: new Date().getFullYear().toString(),
          incident_date: form.incidentDate, incident_time: form.incidentTime,
          incident_location: form.incidentLocation, description: form.description,
          complainant: { full_name: form.complainantName, phone: form.complainantPhone, address: form.complainantAddress, id_proof_type: form.complainantIdType, id_proof_number: form.complainantIdNumber },
          accused: [{ full_name: form.accusedName, alias: form.accusedAlias, father_name: form.accusedFatherName, age: form.accusedAge, physical_description: form.accusedDescription }],
          witnesses: form.witnessName.trim() ? [{ full_name: form.witnessName, phone: form.witnessPhone, statement: form.witnessStatement }] : [],
          seized_items: form.seizedItems.filter(i => i.item.trim()),
          sections_applied: sections.map(s => `${s.law} Section ${s.section} - ${s.title}`),
          officer_name: user?.fullName || 'Unknown', officer_rank: user?.rank || 'IO', officer_badge: user?.badgeNumber || '',
        });
        const pdfResult = await window.crimeGPT.saveAsPDF(firHTML, `FIR_${result.fir_number}.pdf`);
        if (pdfResult.success) {
          await window.crimeGPT.saveDocRecord(result.caseId, 'FIR', 'First Information Report', pdfResult.path);
          setFirDownloaded(true);
        }
      ...
      <div className="flex h-screen bg-white">
        ...
        <main className="flex-1 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #FFF8F0 0%, #FFFFFF 40%, #F0FFF4 100%)' }}>
          <div className="text-center max-w-md">
            <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6"><Check size={36} className="text-green-500" /></div>
            <h2 className="text-xl font-bold text-gray-800 mb-2">Case Registered Successfully</h2>
            <p className="text-gray-500 text-sm mb-1">FIR Number: <span className="font-bold text-gray-800">{caseResult.fir_number}</span></p>
            {firDownloaded && <p className="text-green-600 text-xs mb-6 flex items-center justify-center gap-1"><Check size={12} /> FIR PDF saved to Downloads folder</p>}
            <div className="flex gap-3 justify-center">
              <button onClick={() => onNavigate('dashboard')} className="px-5 py-3 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200 transition">Go to Dashboard</button>
              <button onClick={() => onNavigate('case-detail', caseResult.caseId)} className="px-5 py-3 bg-gradient-to-r from-orange-400 to-green-500 text-white rounded-xl font-medium hover:from-orange-500 hover:to-green-600 transition shadow-lg shadow-orange-200">View Case Details</button>
            </div>
          </div>
        </main>
      </div>
    ...
    <div className="flex h-screen bg-white">
      ...
      <main className="flex-1 overflow-auto" style={{ background: 'linear-gradient(135deg, #FFF8F0 0%, #FFFFFF 40%, #F0FFF4 100%)' }}>
        <header className="bg-white/80 backdrop-blur-xl border-b border-gray-100 px-8 py-4 sticky top-0 z-20 flex items-center justify-between">
          <div className="flex items-center gap-4"><button onClick={() => onNavigate('dashboard')} className="p-2 hover:bg-gray-50 rounded-xl transition text-gray-400 hover:text-gray-600"><ArrowLeft size={20} /></button><div><h2 className="text-lg font-semibold text-gray-800">New FIR Entry</h2><p className="text-xs text-gray-400">Step {step} of 3</p></div></div>
        </header>
        <div className="px-8 pt-6 pb-4"><div className="max-w-3xl mx-auto"><div className="flex items-center justify-between relative mb-6"><div className="absolute top-4 left-[15%] right-[15%] h-0.5 bg-gray-100"><div className="h-full bg-gradient-to-r from-orange-400 to-green-500 rounded-full transition-all duration-500" style={{ width: `${((step - 1) / 2) * 100}%` }} /></div><StepPill n={1} label="FIR Details" active={step === 1} done={step > 1} /><StepPill n={2} label="Parties & Evidence" active={step === 2} done={step > 2} /><StepPill n={3} label="Review & Save" active={step === 3} done={step > 3} /></div></div></div>
        <div className="px-8 pb-24"><div className="max-w-3xl mx-auto space-y-4">
          {step === 1 && (<>
    ...
    <div className="flex items-center justify-between">
      ...
      <button onClick={autoFillPartiesWithAI} disabled={aiFilling}
        ...
        {aiFilling ? 'Filling...' : 'Auto-Fill All'}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm"><h3 className="text-base font-semibold text-gray-800 mb-5">Evidence Photos</h3><input ref={fileRef} type="file" multiple accept="image/*" onChange={handleFiles} className="hidden"/><div onClick={()=>fileRef.current?.click()} className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center cursor-pointer hover:border-orange-300 transition"><Upload size={32} className="mx-auto mb-3 text-gray-300"/><p className="text-sm text-gray-500">Click to upload evidence images</p><p className="text-xs text-gray-400 mt-1">JPG, PNG up to 10MB</p></div>{form.evidenceFiles.length>0&&<div className="grid grid-cols-4 gap-3 mt-4">{form.evidenceFiles.map((f,i)=>(<div key={i} className="relative bg-gray-50 rounded-xl aspect-square flex items-center justify-center border border-gray-200"><img src={f.url} alt={f.name} className="w-full h-full object-cover rounded-xl"/><button onClick={()=>removeFile(i)} className="absolute top-1 right-1 w-6 h-6 bg-red-500 rounded-full flex items-center justify-center"><X size={12} className="text-white"/></button></div>))}</div>}</div>
            <div className="flex justify-between"><button onClick={back} className="px-6 py-3 border border-gray-200 rounded-xl text-gray-600 font-medium hover:bg-gray-50 transition flex items-center gap-2"><ArrowLeft size={18}/> Back</button><button onClick={next} className="px-6 py-3 bg-gradient-to-r from-orange-400 to-green-500 text-white rounded-xl font-medium hover:from-orange-500 hover:to-green-600 transition flex items-center gap-2 shadow-lg shadow-orange-200">Review & Save <ArrowRight size={18}/></button></div>
          </>)}
          {step === 3 && (<>
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm"><h3 className="text-base font-semibold text-gray-800 mb-5">Case Summary</h3><div className="space-y-3 text-sm">{[['Case Type',form.caseType],['Date & Time',`${form.incidentDate} ${form.incidentTime}`],['Location',form.incidentLocation],['Complainant',form.complainantName],['Accused',form.accusedName||'Unknown']].map(([l,v])=>(<div key={l} className="flex gap-4"><span className="text-gray-400 w-28">{l}</span><span className="text-gray-700 font-medium">{v||'—'}</span></div>))}{aiSections&&!aiSections.error&&<div className="pt-2"><span className="text-gray-400">AI Suggested Sections</span><pre className="text-xs text-green-700 mt-1 font-mono">{aiSections.summary}</pre></div>}<div className="pt-2"><p className="text-gray-400 mb-1">Description</p><p className="text-gray-600 text-xs leading-relaxed">{form.description}</p></div></div></div>
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm"><h3 className="text-base font-semibold text-gray-800 mb-4">FIR Document</h3><div className="flex items-center gap-2 py-2 px-3 bg-green-50 rounded-lg border border-green-100"><Check size={14} className="text-green-500"/><span className="text-xs text-green-700 font-medium">First Information Report (FIR) — Auto-generated on save</span></div></div>
            <div className="flex justify-between"><button onClick={back} className="px-6 py-3 border border-gray-200 rounded-xl text-gray-600 font-medium hover:bg-gray-50 transition flex items-center gap-2"><ArrowLeft size={18}/> Back</button><button onClick={handleSave} disabled={saving} className="px-6 py-3 bg-gradient-to-r from-orange-400 to-green-500 text-white rounded-xl font-medium hover:from-orange-500 hover:to-green-600 transition flex items-center gap-2 shadow-lg shadow-orange-200 disabled:opacity-60">{saving?<Loader2 size={18} className="animate-spin"/>:<FileText size={18}/>}{saving?'Saving...':'Save Case & Generate FIR'}</button></div>
          </>)}
        </div></div>
      </main>
    </div>
